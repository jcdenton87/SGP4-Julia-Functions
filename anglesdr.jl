...
function anglesdr(decl1,decl2,decl3,rtasc1,rtasc2,
...
#mu = 1.32712428e11;
   ...
   while (abs(magr1in-magr1old) > tol || abs(magr2in-magr2old) > tol)
       ktr = ktr + 1;
       @sprintf("%2i ",ktr);
       r2,r3,f1,f2,q1,magr1,magr2,a,deltae32 = doubler(cc1,cc2,magrsite1,magrsite2,magr1in,magr2in,                     los1,los2,los3,rsite1,rsite2,rsite3,t1,t3,direct, re, mu);
       ...
       p,a,ecc,incl,omega,argp,nu,m,arglat,truelon,lonper= rv2coeh(r2,v2, re, mu);
         println(@sprintf("coes %11.4f %11.4f %13.9f %13.7f %11.5f %11.5f %11.5f %11.5f",p,a,ecc,incl*rad,
          omega*rad,argp*rad,nu*rad,m*rad ));
...
#         deltar2 = sign(deltar2)*magr2in*pctchg;
       ...
       @sprintf("qs %11.7f  %11.7f  %11.7f \n",q1,q2,q3);
       @sprintf("magr1o %11.7f delr1 %11.7f magr1 %11.7f %11.7f  \n",magr1o,deltar1,magr1in,magr1old);
       @sprintf("magr2o %11.7f delr2 %11.7f magr2 %11.7f %11.7f  \n",magr2o,deltar2,magr2in,magr2old);
